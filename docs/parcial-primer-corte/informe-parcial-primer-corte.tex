...
(cálculo automático de valor según peso/categoría). Al generar la guía se produce además un rótulo
de despacho imprimible, con código de barras, sello de advertencia según categoría de mercancía
(frágil, perecedera, valores) y los datos de remitente/destinatario para bodega.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.62\linewidth]{capturas/gui-08-mensajeria-rotulo.png}
  \caption{Rótulo de despacho imprimible generado al admitir una guía de categoría Frágil, con
  código de barras, sello de advertencia y datos de remitente/destinatario}
  \label{fig:gui-rotulo}
\end{figure}
...
Cada afirmación hecha en esta sección se contrastó directamente contra el código fuente real (no
solo contra lo que se ve en una captura) antes de publicarse aquí. La tabla siguiente resume esa
trazabilidad punto por punto:

{\small\rowcolors{2}{tablestripe}{white}
\begin{longtable}{L{3.7cm} L{4.9cm} L{5.9cm}}
\toprule
\rowcolor{tablehead}\textbf{Requisito} & \textbf{Afirmación en el informe} & \textbf{Verificado en} \\
\midrule
\endhead
\textbf{RF01} --- un tiquete por silla por viaje & Mapa de asientos por piso (VIP Cama / Confort), distingue libre/ocupada & \texttt{SeatMap.tsx}; Fig.~\ref{fig:gui-mapa-asientos} \\
\textbf{RF03} --- bloqueo temporal de silla & Temporizador de 3 minutos al seleccionar una silla & \texttt{SeatMap.tsx} (estado \texttt{tiempoRestante}, 180 s reales por \texttt{setInterval}); Fig.~\ref{fig:gui-rf03-timer} (temporizador activo, silla 1) \\
\textbf{RF04} --- factura debe emitir CUFE & Pase de abordaje con código QR y CUFE simulado al confirmar la venta & \texttt{TiquetesModule.tsx} (modal \texttt{tiqueteBoleto}); Fig.~\ref{fig:gui-boleto}, coherente con la simulación declarada en el propio texto de la GUI \\
\textbf{RF05} --- cálculo automático de valor según peso/categoría & Cubicaje calcula peso volumétrico en tiempo real, alerta de sobrepeso, liquidación dinámica de flete & \texttt{MensajeriaModule.tsx}: fórmula de peso volumétrico, componente \texttt{Alert} condicional y cálculo de tarifa, recargo y total; Fig.~\ref{fig:gui-mensajeria} \\
Estados de \texttt{Tiquete} y \texttt{GuiaEnvio} & Stepper visual con los mismos 5 estados de cada diagrama, en el mismo orden & \texttt{TrazabilidadTab.tsx} contra \texttt{estado-tiquete.puml} / \texttt{estado-guia-envio.puml} --- nombres y orden idénticos 1:1; las transiciones \texttt{SIGUIENTES\_ESTADOS} de \texttt{MensajeriaModule.tsx} replican exactamente las flechas del diagrama de \texttt{GuiaEnvio} \\
Trazabilidad --- KPIs operativos & 4 KPIs en tiempo real (tiquetes, recaudo, guías, novedades) & \texttt{TrazabilidadTab.tsx}: calculados con funciones de arreglo estándar sobre el estado real de \texttt{DataContext}, no hardcodeados \\
\textbf{RNF01} --- control de acceso por cargo & Acceso al workspace operativo identificado por rol & \texttt{Login.tsx} exige elegir 1 de los 3 roles y \texttt{DashboardLayout.tsx} exige sesión activa; el rol queda visible durante toda la sesión (Fig.~\ref{fig:gui-login}) \\
Home --- portal de inicio & Buscador de pasajes y rastreo de encomiendas & \texttt{Home.tsx} (tabs \texttt{pasajes}/\texttt{mensajeria}, \texttt{handleRastrearGuia}); Fig.~\ref{fig:gui-home} \\
\bottomrule
\end{longtable}
}
