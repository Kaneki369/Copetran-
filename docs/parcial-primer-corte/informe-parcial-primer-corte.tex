...
\textbf{Reglas de negocio} & RF05 (cálculo automático de valor según peso/categoría, seguimiento de estado), RF11 (la admisión queda asociada al operario responsable), RF17 (los totales de la remesa se mantienen consistentes a partir de sus guías asociadas mediante \texttt{RemesaDAO.recalcularTotales()}). \\
...
\emph{Nota:} este diagrama formaliza el comportamiento real descrito en el Proceso B del CR-1 T-2, y
sirve como insumo de diseño para parametrizar el campo \texttt{estado\_viaje} en
\texttt{MULTITABLA\_PARAMETRO} en una futura iteración.
...
Operario de Bodega) y requiere sesión activa para entrar a \texttt{/dashboard}, coherente con RNF01
(control de acceso por cargo). El nombre y el rol seleccionado quedan visibles en el panel lateral
durante toda la sesión, de modo que cada pantalla del workspace es trazable al rol que la está
operando.
...
\begin{figure}[h!]
  ...
  \caption{Acceso al sistema --- identificación por rol (RNF01)}
  \label{fig:gui-login}
\end{figure}
...
guías de envío, guías con novedad) y un stepper visual de los dos diagramas de estados
parametrizados (\texttt{ESTADO\_TIQUETE} y \texttt{ESTADO\_GUIA}) documentados en la Sección
\ref{sec:diagramas-estados}, antes de mostrar las tablas de roles ya vistas en la Sección 2. Los
nombres y el orden de los estados en este stepper son literalmente los mismos que en los diagramas
de estados (verificado 1:1 contra los archivos \texttt{.puml}), y las transiciones de estado se
ejecutan realmente desde los módulos de Tiquetes y Mensajería mediante los botones
``$\rightarrow$ ESTADO''.
...
Cada afirmación hecha en esta sección se contrastó directamente contra el código fuente de
\texttt{frontend/src/} (no solo contra lo que se ve en una captura) antes de publicarse aquí. La
tabla siguiente resume esa trazabilidad punto por punto:

{\small\rowcolors{2}{tablestripe}{white}
\begin{longtable}{L{2.9cm} L{5.6cm} L{6.0cm}}
\toprule
\rowcolor{tablehead}\textbf{Requisito} & \textbf{Afirmación en el informe} & \textbf{Verificado en} \\
\midrule
\endhead
RF01 / RF03 & Mapa de asientos por piso (VIP Cama / Confort), distingue libre/ocupada, bloqueo temporal de 3 min & \texttt{SeatMap.tsx} (estado \texttt{tiempoRestante}, 180 s reales por \texttt{setInterval}); Fig.~\ref{fig:gui-mapa-asientos} (mapa) y Fig.~\ref{fig:gui-rf03-timer} (temporizador activo, silla 1) \\
RF04 & Pase de abordaje con código QR y CUFE simulado al confirmar la venta & \texttt{TiquetesModule.tsx} (modal \texttt{tiqueteBoleto}); Fig.~\ref{fig:gui-boleto}, coherente con la simulación declarada en el propio texto de la GUI \\
RF05 & Cubicaje calcula peso volumétrico en tiempo real, alerta de sobrepeso, liquidación dinámica de flete & \texttt{MensajeriaModule.tsx}: fórmula de peso volumétrico, componente \texttt{Alert} condicional y cálculo de tarifa, recargo y total; Fig.~\ref{fig:gui-mensajeria} \\
Estados \texttt{ESTADO\_TIQUETE} / \texttt{ESTADO\_GUIA} & Stepper visual con los mismos 5 estados de cada diagrama, en el mismo orden & \texttt{TrazabilidadTab.tsx} contra \texttt{estado-tiquete.puml} / \texttt{estado-guia-envio.puml} --- nombres y orden idénticos 1:1; las transiciones \texttt{SIGUIENTES\_ESTADOS} de \texttt{MensajeriaModule.tsx} replican exactamente las flechas del diagrama de \texttt{GuiaEnvio} \\
Trazabilidad --- KPIs & 4 KPIs en tiempo real (tiquetes, recaudo, guías, novedades) & \texttt{TrazabilidadTab.tsx}: calculados con funciones de arreglo estándar sobre el estado real de \texttt{DataContext}, no hardcodeados \\
RNF01 & Acceso al workspace operativo identificado por rol & \texttt{Login.tsx} exige elegir 1 de los 3 roles y \texttt{DashboardLayout.tsx} exige sesión activa; el rol queda visible durante toda la sesión (Fig.~\ref{fig:gui-login}) \\
Home --- portal & Buscador de pasajes y rastreo de encomiendas & \texttt{Home.tsx} (tabs \texttt{pasajes}/\texttt{mensajeria}, \texttt{handleRastrearGuia}); Fig.~\ref{fig:gui-home} \\
\bottomrule
\end{longtable}
}
...
\clearpage
% ---------------------------------------------------------------------------
\section{Notas de Trazabilidad}
% ---------------------------------------------------------------------------

\begin{itemize}
  \item El diagrama de estados de \texttt{ViajeProgramado} formaliza el ciclo de vida completo de
  un viaje (\texttt{PROGRAMADO} $\rightarrow$ \texttt{EN\_ALISTAMIENTO} $\rightarrow$
  \texttt{EN\_CURSO} $\rightarrow$ \texttt{FINALIZADO}/\texttt{CANCELADO}), consistente con el
  Proceso B del CR-1 T-2 --- se documenta explícitamente como aporte de diseño de este parcial.
  \item El diagrama de clases DAO incorpora \texttt{recalcularTotales()} en
  \texttt{RemesaDAO}/\texttt{Remesa} como método de negocio explícito, coherente con RF17
  (mantenimiento de totales consistentes de la remesa a partir de sus guías asociadas).
  ...
  memoria. Ver capturas y matriz de trazabilidad en la Sección \ref{sec:gui-implementada},
  disponible para demostración en vivo durante la sustentación.
\end{itemize}
