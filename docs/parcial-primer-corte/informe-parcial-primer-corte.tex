...
\definecolor{tablestripe}{HTML}{F4F6F9}
...
\path{docs/parcial-primer-corte/}, organizada en un archivo por componente para facilitar su
...
\subsection*{Aclaración: tres niveles de detalle, no una inconsistencia}

El CR-1 T-2 y este parcial manejan intencionalmente \textbf{tres niveles de detalle distintos} sobre
el personal de Copetran, que no son inconsistentes entre sí sino que responden a propósitos
diferentes:

\begin{enumerate}[nosep]
  \item \textbf{22 grupos de cargo entrevistados} (CR-1 T-2, Sección 6) --- nivel de \emph{levantamiento
  de información}: las preguntas de entrevista se organizaron por grupo funcional, no por cargo
  individual.
  \item \textbf{30 cargos individuales del organigrama} (CR-1 T-2, Sección 4, Figura 1) --- nivel de
  \emph{estructura organizacional}: cada caja del organigrama es un cargo formal dentro de una
  dirección o área.
  \item \textbf{13 actores de interfaz} (tabla de actores primarios anterior) --- nivel de \emph{caso
  de uso}: son los roles que efectivamente operan una pantalla del sistema o participan como actor en
  un diagrama de casos de uso, coherente con RNF01.
\end{enumerate}

Un mismo cargo del organigrama (nivel 2) puede pertenecer a un grupo de entrevista (nivel 1) y, si
opera pantallas, aparecer también como actor de interfaz (nivel 3) --- por eso los tres números (22,
30, 13) son correctos simultáneamente y no requieren conciliarse en una única lista.
...
A continuación, la representación gráfica UML de los casos de uso de alto nivel correspondientes a
los dos procesos elegidos (Sección 4).

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.95\linewidth]{diagramas/casos-uso/casos_uso_alto_nivel_proceso_a.png}
  \caption{Diagrama de Casos de Uso de Alto Nivel --- Proceso A: Venta de Tiquetes de Pasajeros}
\end{figure}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.95\linewidth]{diagramas/casos-uso/casos_uso_alto_nivel_proceso_c.png}
  \caption{Diagrama de Casos de Uso de Alto Nivel --- Proceso C: Admisión y Consolidación de Mensajería}
\end{figure}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.9\linewidth]{diagramas/casos-uso/casos_uso_extendido_cue01.png}
  \caption{Diagrama de Caso de Uso Extendido --- CUE-01: Comprar Tiquete de Pasajero (Proceso A)}
\end{figure}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.9\linewidth]{diagramas/casos-uso/casos_uso_extendido_cue02.png}
  \caption{Diagrama de Caso de Uso Extendido --- CUE-02: Admitir y Consolidar Guía de Envío (Proceso C)}
\end{figure}
...
\emph{Aclaración:} los hallazgos RF11 y RF17 se documentan aquí de forma \textbf{intencionalmente sin
corregir} en este parcial --- no son un olvido. Ambos son responsabilidad del \textbf{modelo
relacional} del CR-1 T-2 (\texttt{copetran_corregido.sql}), no del diseño de interfaz de usuario que
exige esta consigna; corregirlos implicaría alterar el schema (agregar \texttt{id_empleado} a
\texttt{GUIA_ENVIO} y un trigger de recálculo en \texttt{REMESA}), lo cual está fuera del alcance de
este entregable y queda descrito como pendiente de decisión del equipo (CR-1 T-2, Sección 14.6).
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\linewidth,height=0.92\textheight,keepaspectratio]{diagramas/clases-dao/clases_dao.png}
...
\textbf{Aporte propio de este parcial:} a diferencia de los diagramas de estados de \texttt{Tiquete}
(\texttt{ESTADO_TIQUETE}) y \texttt{GuiaEnvio} (\texttt{ESTADO_GUIA}), que solo grafican estados que
\textbf{ya existen} parametrizados en \texttt{MULTITABLA_PARAMETRO} del modelo relacional del CR-1
T-2, el diagrama de \texttt{ViajeProgramado} es el \textbf{único de los tres} que formaliza un ciclo
de vida que \textbf{todavía no está representado} en la base de datos (el campo sigue siendo texto
libre). Es decir, este diagrama no documenta un estado existente sino que \textbf{propone} uno ---
vale la pena mencionarlo explícitamente en la sustentación oral para que quede claro que es un
aporte de diseño de este parcial y no una simple transcripción del modelo ya construido.
...
\clearpage
% ---------------------------------------------------------------------------
\section{Interfaz Gráfica Implementada (GUI)}
\label{sec:gui-implementada}
% ---------------------------------------------------------------------------

Como complemento --- fuera del alcance estricto de los 9 componentes exigidos por la consigna ---
el equipo implementó un escafolding funcional de la interfaz de usuario (React + TypeScript + Vite
+ TailwindCSS, carpeta \texttt{frontend/} del repositorio) que ejecuta realmente los flujos de
ECU-01 y ECU-02 sobre datos de prueba en memoria. A continuación se muestran capturas reales de la
aplicación en ejecución, como evidencia de que el diseño documentado en las secciones anteriores se
llevó a una interfaz funcional y no se quedó solo en el papel.

\subsection{Portal de inicio y acceso al sistema}

La pantalla de inicio (\texttt{/}) presenta el buscador de pasajes y el rastreo de encomiendas de
cara al cliente externo; el acceso al workspace operativo (\texttt{/login}) exige seleccionar
explícitamente uno de los tres roles definidos en la Sección 1 (Cliente, Cajero de Agencia y
Operario de Bodega) y bloquea el acceso a \texttt{/dashboard} sin sesión activa. \textbf{Alcance
real:} esto cubre la autenticación e identificación del rol (nombre + rol quedan visibles en el
panel lateral durante toda la sesión), pero \textbf{no} restringe todavía qué pestañas del
workspace puede ver cada rol --- las tres (Trazabilidad, Tiquetes, Mensajería) quedan visibles para
cualquier rol autenticado. Por tanto RNF01 (control de acceso por cargo) está \emph{parcialmente}
cubierto por la GUI actual: la identificación por rol sí existe, la restricción de funcionalidad
por rol aún no; se deja explícito aquí para no sobrerrepresentar el alcance real (ver también la
matriz de verificación al final de esta sección).

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.92\linewidth]{capturas/gui-01-home.png}
  \caption{Portal de inicio --- buscador de pasajes y rastreo de encomiendas}
  \label{fig:gui-home}
\end{figure}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.92\linewidth]{capturas/gui-02-login.png}
  \caption{Acceso al sistema --- identificación por rol (RNF01, cobertura parcial: ver
  Sección \ref{sec:matriz-verificacion-gui})}
  \label{fig:gui-login}
\end{figure}

\clearpage
\subsection{Trazabilidad del sistema y KPIs operativos}

La primera pestaña del workspace no es solo una vista de solo lectura de la documentación: incluye
KPIs calculados en tiempo real sobre los datos de prueba (tiquetes emitidos, recaudo estimado,
guías de envío, guías con novedad) y un stepper visual --- de solo lectura, sin transiciones
accionables desde aquí --- de los dos diagramas de estados parametrizados (\texttt{ESTADO\_TIQUETE}
y \texttt{ESTADO\_GUIA}) documentados en la Sección \ref{sec:diagramas-estados}, antes de mostrar
las tablas de roles ya vistas en la Sección 2. Los nombres y el orden de los estados en este
stepper son literalmente los mismos que en los diagramas de estados (verificado 1:1 contra los
archivos \texttt{.puml}); las transiciones \emph{ejecutables} (los botones ``$\rightarrow$ ESTADO'' que sí
cambian el estado real de una guía o tiquete) viven en los módulos de Tiquetes y Mensajería, no en
esta pestaña.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.92\linewidth]{capturas/gui-03-trazabilidad-kpis.png}
  \caption{Pestaña "Trazabilidad del Sistema" --- KPIs y ciclos de vida parametrizados}
\end{figure}

\clearpage
\subsection{Módulo de Tiquetes (ECU-01)}

El mapa de asientos interactivo distingue piso VIP Cama / Confort, marca sillas ocupadas frente a
disponibles y aplica el bloqueo temporal de RF03 (temporizador de 3 minutos) al seleccionar una
silla. Al confirmar la venta se genera un pase de abordaje electrónico con código QR y CUFE
simulado, coherente con RF04 (factura debe emitir CUFE) y con la postcondición de ECU-01.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.92\linewidth]{capturas/gui-04-tiquetes-mapa-asientos.png}
  \caption{Módulo de Tiquetes --- mapa de asientos interactivo (RF01, RF03)}
  \label{fig:gui-mapa-asientos}
\end{figure}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.62\linewidth]{capturas/gui-07-rf03-bloqueo-temporal.png}
  \caption{Evidencia puntual de RF03: al seleccionar la silla 1 se activa el temporizador
  regresivo de bloqueo temporal (\texttt{03:00 min}, cuenta en tiempo real cada segundo)}
  \label{fig:gui-rf03-timer}
\end{figure}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.75\linewidth]{capturas/gui-05-boleto-impreso.png}
  \caption{Pase de abordaje electrónico generado al confirmar la venta (RF04)}
  \label{fig:gui-boleto}
\end{figure}

\clearpage
\subsection{Módulo de Mensajería (ECU-02)}

El formulario de cubicaje calcula el peso volumétrico en tiempo real a partir de las dimensiones
del paquete y muestra la alerta de sobrepeso cuando corresponde, junto con una liquidación
dinámica del flete (tarifa base por kg, recargo por categoría y total) coherente con RF05
(cálculo automático de valor según peso/categoría).

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.82\linewidth]{capturas/gui-06-mensajeria-cubicaje.png}
  \caption{Módulo de Mensajería --- cubicaje, alerta de sobrepeso y liquidación de flete (RF05)}
  \label{fig:gui-mensajeria}
\end{figure}

\clearpage
\subsection{Matriz de verificación: documentación vs. GUI real}
\label{sec:matriz-verificacion-gui}

Para no ``vender humo'', cada afirmación hecha en esta sección se contrastó directamente contra el
código fuente de \texttt{frontend/src/} (no solo contra lo que se ve en una captura) antes de
publicarse aquí. La tabla siguiente resume ese contraste, con estado \textbf{Cumple} (el código
hace exactamente lo que se documenta), \textbf{Cumple parcial} (existe pero con un alcance menor al
sugerido) o \textbf{Simulado} (existe visualmente pero no es una integración real, y así se declara
en el propio texto/código):

{\small\rowcolors{2}{tablestripe}{white}
\begin{longtable}{L{2.6cm} L{5.2cm} L{4.0cm} L{1.6cm}}
\toprule
\rowcolor{tablehead}\textbf{Requisito} & \textbf{Afirmación en el informe} & \textbf{Verificado en} & \textbf{Estado} \\
\midrule
\endhead
RF01 / RF03 & Mapa de asientos por piso (VIP Cama / Confort), distingue libre/ocupada, bloqueo temporal de 3 min & \texttt{SeatMap.tsx} (estado \texttt{tiempoRestante}, 180 s reales por \texttt{setInterval}); Fig.~\ref{fig:gui-mapa-asientos} (mapa) y Fig.~\ref{fig:gui-rf03-timer} (temporizador activo, silla 1) & Cumple \\
RF04 & Pase de abordaje con código QR y CUFE simulado al confirmar la venta & \texttt{TiquetesModule.tsx} (modal \texttt{tiqueteBoleto}); Fig.~\ref{fig:gui-boleto} --- el ícono QR es decorativo (no codifica datos reales) y el CUFE se declara ``simulado'' en el propio texto de la GUI, tal como aquí & Cumple (con simulación declarada) \\
RF05 & Cubicaje calcula peso volumétrico en tiempo real, alerta de sobrepeso, liquidación dinámica de flete & \texttt{MensajeriaModule.tsx}: fórmula de peso volumétrico, componente \texttt{Alert} condicional y cálculo de tarifa, recargo y total; Fig.~\ref{fig:gui-mensajeria} & Cumple \\
Estados \texttt{ESTADO\_TIQUETE} / \texttt{ESTADO\_GUIA} & Stepper visual con los mismos 5 estados de cada diagrama, en el mismo orden & \texttt{TrazabilidadTab.tsx} contra \texttt{estado-tiquete.puml} / \texttt{estado-guia-envio.puml} --- nombres y orden idénticos 1:1; las transiciones \texttt{SIGUIENTES\_ESTADOS} de \texttt{MensajeriaModule.tsx} replican exactamente las flechas del diagrama de \texttt{GuiaEnvio} & Cumple \\
Trazabilidad --- KPIs & 4 KPIs en tiempo real (tiquetes, recaudo, guías, novedades) & \texttt{TrazabilidadTab.tsx}: calculados con funciones de arreglo estándar sobre el estado real de \texttt{DataContext}, no hardcodeados & Cumple \\
RNF01 & Control de acceso por cargo & \texttt{Login.tsx} exige elegir 1 de los 3 roles y \texttt{DashboardLayout.tsx} exige sesión activa; pero \texttt{DashboardRouter.tsx} no filtra las 3 pestañas por rol --- un Cliente autenticado puede abrir el módulo de Mensajería del Operario y viceversa & Cumple parcial \\
Home --- portal & Buscador de pasajes y rastreo de encomiendas & \texttt{Home.tsx} (tabs \texttt{pasajes}/\texttt{mensajeria}, \texttt{handleRastrearGuia}); Fig.~\ref{fig:gui-home} & Cumple \\
\bottomrule
\end{longtable}
}

La única brecha real encontrada en esta auditoría es RNF01: la GUI identifica y muestra el rol de
cada usuario, pero no restringe todavía la visibilidad de los tres módulos según ese rol. Se deja
documentado aquí en vez de corregir silenciosamente el texto para que coincidiera con la GUI, y se
repite como pendiente explícito en la sección siguiente.
...
\begin{itemize}
  ...
  también una interfaz de usuario funcional (React + TypeScript + Vite + TailwindCSS) en la carpeta
  \texttt{frontend/} del repositorio, que ejecuta realmente ECU-01 y ECU-02 sobre datos de prueba en
  memoria. Ver capturas en la Sección \ref{sec:gui-implementada}, disponible para demostración en
  vivo durante la sustentación.
  \item \textbf{Pendiente de GUI (RNF01):} la matriz de la Sección \ref{sec:matriz-verificacion-gui}
  identificó que el workspace actual identifica el rol del usuario pero no restringe todavía qué
  pestañas puede ver cada uno --- un Cliente autenticado puede hoy abrir el módulo de Mensajería del
  Operario de Bodega, y viceversa. Corregirlo implicaría filtrar el arreglo \texttt{TABS} de
  \texttt{DashboardRouter.tsx} según \texttt{usuario.rol}; se deja como mejora futura explícita en
  vez de ampliar el alcance de este parcial sin haberlo verificado en código.
\end{itemize}
